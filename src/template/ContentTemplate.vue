...
import { theme, changeTheme, themeBoolean } from '@/controller/theme'
...
<template>
  <template v-if="themeBoolean">
    <!-- header start -->
    <div
      class="bg-gradient-to-r fixed w-full from-purple-300 via-purple-200 to-sky-200 h-16 flex items-center px-5 justify-between ">
      ...
      <div class="float-end ml- hidden lg:inline-block relative" @click="gearRotate">
        ...
        <transition enter-active-class="transition-all duration-300 ease-out"
          ...
          <div v-if="gear" class="absolute bg-neutral-300 w-50 top-11 -right-5 rounded-bl-2xl">
            ...
            <div class="text-center bg-neutral-300 h-10 flex justify-center items-center">
              <div :class="['rounded-2xl w-10 h-5 border', themeBoolean ? 'bg-neutral-300' : 'bg-neutral-800']"
                @click="changeTheme">
                <div
                  :class="['rounded-full h-5 w-5', themeBoolean ? 'bg-neutral-700' : 'bg-neutral-200 translate-x-5']">
                </div>
              </div>
            </div>
          </div>
        ...
        <transition enter-active-class="transition duration-300 ease-in-out"
          ...
          <div v-if="mobileHeader" class="bg-neutral-200 text-gray-800 absolute top-11 -right-5 shadow-lg">
            <ul class="p-2">
              ...
              <li class="cursor-pointer underline" @click="changeTheme"> {{ theme }} </li>
            </ul>
          </div>
        </transition>
          ...
          <transition enter-active-class="transition-all duration-500 ease-out"
            ...
            <div v-if="key.isOpen" class="overflow-hidden">
              <li v-for="(j, jIndex) in key.list" :key="jIndex"
                class="pl-6 bg-yellow-100 hover:bg-yellow-200 cursor-pointer rounded">
                <span class="">-</span>
                <router-link :to="j.route">{{ j.listMenu }}</router-link>
              </li>
            </div>
          </transition>
        </ul>
      ...
      <ul>
        <div v-for="(item, index) in sideContent" :key="index">
          <div class="grid grid-cols-5">
            ...
            <div class="mt-2 w-10 text-2xl flex justify-center text-gray-400">
              <i :class="item.icon"></i>
            </div>
            <!-- Head List + Dropdown -->
            <transition enter-active-class="transition-all duration-500 ease-out"
              ...
              <div v-if="sideOver" class="mt-2 w-40 text-[15px]">
                ...
                <div @click="toggleList(index)" class="cursor-pointer hover:text-blue-800 font-medium">
                  {{ item.headList }}
                </div>
                ...
                <transition enter-active-class="transition-all duration-300 ease-in-out"
                  ...
                  <div v-if="item.isOpen" class="overflow-hidden mt-1 ml-2 space-y-1">
                    <div v-for="(key, i) in item.list" :key="i" class="text-sm hover:underline cursor-pointer">
